...
const Question = ({ id, question }) => {
    ...
    useEffect(() => {
        ...
        if (isDropdownActive && goodAnswers.length < 1) {
            Api('suggestions/fetchAnswers?' + new URLSearchParams({ question: id })).then(res => {
    ...
    const acceptQuestion = () => {
        Api('suggestions/accept?' + new URLSearchParams({ question: id })).then(() => {
    ...
    const deleteQuestion = () => {
        Api('suggestions/delete?' + new URLSearchParams({ question: id })).then(() => {
            setDropdownActive(false);
            setTimeout(() => window.location.reload(), 100);
        }).catch(console.error);
    };
            ...
            <div className="question-dropdown">
                {wrongAnswers.length > 0 ? (
                    <>
                        ...
                        <button className='btn' style={{ background: 'green', margin: '.5rem' }} onClick={acceptQuestion}>Zaakceptuj</button>
                        <button className='btn' style={{ background: 'red', margin: '.5rem' }}onClick={deleteQuestion}>Usuń</button>
                    </>
                ) : (
...
const QuestionPropositions = () => {
    ...
    useEffect(() => {
        ...
        Api('suggestions/getAmmount').then(res => {
...
export default QuestionPropositions;
